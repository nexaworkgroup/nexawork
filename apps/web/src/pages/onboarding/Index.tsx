...
export default function OnboardingPage() {
  ...
  const [form, setForm] = useState({
    full_name: '', location: '', degree: '', field_of_study: '',
    institution: '', graduation_year: '', bio: ''
  })
  ...
  const [empForm, setEmpForm] = useState({
    company_name: '', industry: '', company_size: '', location: '', website: '', description: ''
  })
  ...
  const canProceed = () => {
    if (isEmployer) {
      ...
      return empForm.description.trim().length > 0
    }
            ...
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={i} className={clsx(
                ...
                i + 1 < step ? 'border-brand-green bg-brand-green text-white'
                  : i + 1 === step ? 'border-brand-green text-brand-green'
        ...
        <div className="card">
          {!isEmployer ? (
            <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))}
                        className="input-field" placeholder="Yaoundé, Cameroon" />
                    </div>
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  ...
                  <p className="text-sm text-gray-400 mb-6">This helps us match you with the right opportunities</p>
                  <div className="space-y-4">
                    <div>
                      ...
                      <input value={form.degree} onChange={e => setForm(p => ({ ...p, degree: e.target.value }))}
                        className="input-field" placeholder="BSc, MSc, HND, BTS..." />
                    </div>
                    <div>
                      ...
                      <input value={form.field_of_study} onChange={e => setForm(p => ({ ...p, field_of_study: e.target.value }))}
                        className="input-field" placeholder="Computer Science, Business..." />
                    </div>
                    <div>
                      ...
                      <input value={form.institution} onChange={e => setForm(p => ({ ...p, institution: e.target.value }))}
                        className="input-field" placeholder="University of Yaoundé I" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('onboarding.grad_year')}</label>
                      <input type="number" value={form.graduation_year} onChange={e => setForm(p => ({ ...p, graduation_year: e.target.value }))}
            ...
            <>
              ...
              {step === 1 && (
                <div>
                  <h2 className="text-xl font-bold text-gray-900 mb-1">{t('onboarding.employer_step1_title')}</h2>
                  <div className="space-y-4 mt-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('onboarding.company_name')} *</label>
                      <input value={empForm.company_name} onChange={e => setEmpForm(p => ({ ...p, company_name: e.target.value }))}
                        className="input-field" placeholder="Acme Corp" autoFocus />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('onboarding.industry')}</label>
                      <input value={empForm.industry} onChange={e => setEmpForm(p => ({ ...p, industry: e.target.value }))}
                        className="input-field" placeholder="Technology, Finance, Agriculture..." />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('onboarding.company_size')}</label>
                      <select value={empForm.company_size} onChange={e => setEmpForm(p => ({ ...p, company_size: e.target.value }))}
              ...
              {step === 2 && (
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('onboarding.location')}</label>
                      <input value={empForm.location} onChange={e => setEmpForm(p => ({ ...p, location: e.target.value }))}
                    ...
                    <div>
                      ...
                      <input value={empForm.website} onChange={e => setEmpForm(p => ({ ...p, website: e.target.value }))}
              ...
              {step === 3 && (
                <div>
                  ...
                  <textarea value={empForm.description} onChange={e => setEmpForm(p => ({ ...p, description: e.target.value }))}
          ...
          <div className="flex gap-3 mt-8">
            ...
            {step < totalSteps ? (
              ...
              <button onClick={handleFinish} disabled={loading || !canProceed()} className="btn-gold flex-1 py-3 text-base font-semibold">
